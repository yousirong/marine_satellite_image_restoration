...
#             img = tiff.imread(self.data[index]).astype(np.float32)
#             if img is None:
...
#     def load_land_sea_mask(self, land_mask_path):
#         """
#         Load the land-sea mask from a .npy file and convert values:
#         원본 mask: -1=육지, 0=해양
#         변환 mask:  1=육지, 0=해양
#         """
#         land_sea_mask = np.load(land_mask_path)
#         # -1 → 1(육지), 0 → 0(해양)
#         land_sea_mask = np.where(land_sea_mask == -1, 1, 0).astype(np.uint8)
#         unique_vals = np.unique(land_sea_mask)
#         print(f"Loaded land-sea mask with shape: {land_sea_mask.shape}, dtype: {land_sea_mask.dtype}")
#         print(f"Converted unique values in land_sea_mask: {unique_vals}")  # 이제 [0 1]만 찍힐 것
#         return land_sea_mask
