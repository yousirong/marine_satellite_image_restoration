...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, image_path, mask_path, mask_mode, target_size, augment=False, training=True, mask_reverse = False):
        super(Dataset, self).__init__()
        self.augment = augment
        self.training = training
        self.data = self.load_list(image_path)
        #self.data = self.data[:216]
    ...
    def load_item(self, index):
        #for Grayscale image
        #img = imread(self.data[index],as_gray=False, pilmode="RGB")
        #print(img.shape)
    ...
    def load_mask(self, img, index):
        ...
        if self.mask_type == 2:
            mask_index = index
            try:
                mask_img = cv2.imread(self.mask_data[mask_index])
            except:
                mask_index = random.randrange(0, len(self.mask_data))
                mask_img = cv2.imread(self.mask_data[mask_index])
            #print(mask_img,mask_img.shape)
            mask = cv2.cvtColor(mask_img, cv2.COLOR_BGR2GRAY) #(256,256)
            mask = mask/255
    ...
    def load_list(self, path):
        if isinstance(path, str):
            ...
            if os.path.isdir(path):
                #path = list(glob.glob(path + '/*.tif'))#for DayMOM dataset
                print("PASS??")
                path = list(glob.glob(path + '/*.tiff')) + list(glob.glob(path + '/*.png'))
                path.sort()
                #print(path)
                return path
