...
def plot_parity(true, pred, rmse_, mae_, kind="scatter", 
                ...
                figsize=(8, 8), ax=None, filename='./performance/rrs/modis'):
    ...
    if not ax:
        fig.tight_layout()
        if filename:
            fig.savefig(filename+'40.png')
    else: print("fail@@")
    return ax

#goci_path  = './recon/chl/2021/50/'
...
#modis_path = '/home/pmilab/Documents/VIIRS/2021/443/'
...
modis_files_list = list(glob.glob(os.path.join(modis_path, '*'), recursive=True))
#modis_files_list = modis_files_list.sort()
goci_files_list = list(glob.glob(os.path.join(goci_path, '*'), recursive=True))
#goci_files_list = goci_files_list.sort()
mask_files_list = list(glob.glob(os.path.join(mask_path, '*'), recursive=True))
