...
def plot_parity(true, pred, rmse_, mae_, kind="scatter", 
    ...
    return ax
'''
def plotting(save_path,gt_np, restored_np) : 
    #w = gt_np.shape[0]
    #h = w
    ...
    for i in range(len(gt_np)):
        temp_rmse = temp_rmse + (gt_np[i]-restored_np[i])**2
        temp_mae = temp_mae + abs(gt_np[i]-restored_np[i])
    
    temp_mae = temp_mae/len(gt_np)
    temp_rmse = math.sqrt(temp_rmse/len(gt_np))

    plt_gt = np.array(gt_np)
    plt_res = np.array(restored_np)
    plt.scatter(plt_gt, plt_res, s=10)
    b0, b1 = np.polyfit(plt_gt, plt_res, 1)
    
    print(plt_gt.dtype)
    
    plt.plot(plt_gt, b1+b0*plt_gt, color='red')

    plt.title('MODIS RRS Loss 30-40')
    plt.xlabel('RRS 443')
    plt.ylabel('Model')

    plt.text(0,0.012, 'MAE : '+str(temp_mae) +'\n'+'RMSE : '+str(temp_rmse))
    plt.savefig(save_path+'band443_30.png')
'''
...
#goci_path = '/home/pmilab/Documents/validation_data/recon/GOCI_RRS_degree/2021/2/10/img/'
#modis_path = '/home/pmilab/Documents/MODIS/2021/test'
...
for i in range(len(goci_files_list)):
    ...
    if len(match_list) > 1:
        ...
        #gt_np = cv2.imread(selected_modis, cv2.IMREAD_UNCHANGED)
        #gt_np = gt_np / 255.
