...
# 모든 전처리를 train으로 설정
for pct in pcts:
    temp = os.path.join(save_base, 'train', pct)
...
# 데이터 전처리 (전체 년도에 대해서 처리, 전부 train에 저장)
for year in years:
    ...
    for month in tqdm(months):
        ...
        if os.path.isdir(data_month):
            ...
            for i in range(0, len(imgs) - 7):
                ...
                s_mask_patch = land_sea_mask[x_sae + s_row:x_sae + s_row + 256, y_sae + s_col + s_col + 256]
                ...
                if n_ocean_pct >= min_ocean_pct and n_pct != 100:
                    if n_temp < 1:
                        save_path = os.path.join(save_base, 'train', 'perfect', f"{year}_{month}_{i}_nak.tiff")
                    else:
                        save_path = os.path.join(save_base, 'train', str(n_pct), f"{year}_{month}_{i}_nak.tiff")
                    save_patch_image(n_patch_256, save_path)

                # s_patch 저장 (s_temp와 s_pct에 따라 저장)
                if s_ocean_pct >= min_ocean_pct and s_pct != 100:
                    if s_temp < 1:
                        save_path = os.path.join(save_base, 'train', 'perfect', f"{year}_{month}_{i}_sae.tiff")
                    else:
                        save_path = os.path.join(save_base, 'train', str(s_pct), f"{year}_{month}_{i}_sae.tiff")
